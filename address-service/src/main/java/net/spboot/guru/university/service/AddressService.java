...
public class AddressService {
    ...
    public void registerAddressByzipCode(String personId, String zipCode) {
        ...
        if (zipCode.equals("0000")) {
            // address = Address.builder().personId(personId).city("Tunis").zipCode("0000").build();
        ...
        if (zipCode.equals("0002")) {
            //address = Address.builder().personId(personId).city("Ben Arous").zipCode("0002").build();
            // Address address = new Address(personId, "Ben Arous", "0002");
            log.info("New Address registration {} for Student {} ",  personId);
            addressRepository.saveAndFlush(Address.builder().personId(personId).city("Ben Arous").zipCode("0002").build());
          
        }

        if (zipCode.equals("0003")) {
            //address = Address.builder().personId(personId).city("Berg de Lac").zipCode("0003").build();
           // Address address = new Address(personId, "Berg de Lac", "0003");
            log.info("New Address registration {} for Student {} ",  personId);
            addressRepository.saveAndFlush(Address.builder().personId(personId).city("Berg de Lac").zipCode("0003").build());
            
        }

        if (zipCode.equals("0004")) {
           
           // address =  Address.builder().personId(personId).city("Marsa").zipCode("0004").build();
